...
import { getHealthPolicies, deleteHealthPolicy } from "@/services/healthPolicyService";
import { Edit, Trash2, ChevronLeft, ChevronRight, Loader, AlertCircle, HeartPulse, Search } from "lucide-react";
import DeleteConfirmation from "../delete/DeleteConfirmation";

export default function HealthPolicyList({ refreshTrigger, onEdit }) {
  ...
  const fetchPolicies = async () => {
    ...
    try {
      const data = await getHealthPolicies({ page: currentPage, pageSize, search: searchTerm || undefined });
      const list = data.items || data.healthPolicies || data.data || [];
      ...
      console.error("Failed to fetch health policies:", err);
  ...
  // Filter policies by patient ID when searchTerm changes
  useEffect(() => {
    ...
    const filtered = policies.filter((p) => {
      const name = (p.patientName || p.patientID || p.patientId || "").toString().toLowerCase();
  ...
  return (
    <Card className="overflow-hidden border border-gray-200 rounded-md bg-white dark:bg-gray-800 p-4">
      <div className="flex justify-between mb-4">
        <h2 className="text-xl font-semibold">Health Policy List</h2>
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Health Type</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Min Value</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Max Value</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Health Level </th>
            ...
            <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
              {pageItems.length > 0 ? (
                pageItems.map((p) => (
                  <tr key={p.id || `${p.patientID || p.patientId}-${p.healthType}`} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <HeartPulse className="h-4 w-4 mr-2 text-gray-400" />
                        <div className="text-sm font-medium text-gray-900 dark:text-gray-300">{p.patientName || p.patientID || p.patientId}</div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{p.healthType}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{p.minValue}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{p.maxValue}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{p.healthLevelName}</td>

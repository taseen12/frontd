...
import { useAddRoleForm } from "@/hooks/role/useAddRoleForm";
import { useEditRoleForm } from "@/hooks/role/useEditRoleForm";

export default function AddRoleForm({ onRoleAdded, editingRole, onCancelEdit, onUpdated }) {
  const isEditMode = !!editingRole;

  const addFormState = useAddRoleForm();
  const editFormState = useEditRoleForm(editingRole, onUpdated, onCancelEdit);
    ...
    <div className="w-full bg-white dark:bg-gray-800 p-4 rounded-md border border-gray-200 dark:border-gray-300 space-y-3">
      {success && (
        <div className="p-3 bg-green-50 border border-green-200 text-primaryColor rounded-md text-sm">
          <div className="flex items-center">
            ...
            <span>{isEditMode ? "Role updated successfully!" : "Role added successfully!"}</span>
          </div>
        </div>
      )}
      ...
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-300 text-sm mb-4">
          {isEditMode ? "Update the role name" : "Add a new role to your organization"}
        </p>
      </div>
